...
pub const Terminal = struct {
    ...
    // Theme colors for OSC 10/11 reporting (server-driven, not hardcoded)
    theme_bg: u32 = 0xffffff,
    theme_fg: u32 = 0x000000,
    ...
    pub fn setTheme(self: *Terminal, bg: u32, fg: u32) void {
        self.theme_bg = bg & 0xffffff;
        self.theme_fg = fg & 0xffffff;
    }
    ...
    fn handleOsc(self: *Terminal) void {
        ...
        // Theme-aware reporting: TUI picks dark/light variant by bg luminance at startup
        if (data.len == 4 and data[0] == '1' and data[1] == '1' and data[2] == ';' and data[3] == '?') {
            ...
            const resp = std.fmt.bufPrint(&buf, "\x1b]11;rgb:{x:0>4}/{x:0>4}/{x:0>4}{s}", .{ rr, gg, bb, term }) catch return;
            self.enqueueResponse(resp);
            return;
        }
        if (data.len == 4 and data[0] == '1' and data[1] == '0' and data[2] == ';' and data[3] == '?') {
            var buf: [32]u8 = undefined;
            const r: u16 = @intCast((self.theme_fg >> 16) & 0xff);
            const g: u16 = @intCast((self.theme_fg >> 8) & 0xff);
            const b: u16 = @intCast(self.theme_fg & 0xff);
            const rr = r * 257;
            const gg = g * 257;
            const bb = b * 257;
            const term: []const u8 = if (self.parser.osc_terminated_by_st) "\x1b\\" else "\x07";
            const resp = std.fmt.bufPrint(&buf, "\x1b]10;rgb:{x:0>4}/{x:0>4}/{x:0>4}{s}", .{ rr, gg, bb, term }) catch return;
            self.enqueueResponse(resp);
            return;
        }
