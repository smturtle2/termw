...
- WASM `vendor/wterm/src/terminal.zig:setTheme()` + `wasm_api.zig:setThemeColors()` called by `src/client/main.ts:applyTheme()` (via `luminance(background)>128` → `theme-korean-light` vs `theme-dark`) and by WS live update, so `ESC]11;?`/`10;?` → `rgb:ffff/ffff/ffff` etc + `ST` vs `BEL` echo (`parser.zig:osc_terminated_by_st`) and `ESC]4;idx;?` palette queries (`rgb:RRRR/GGGG/BBBB` from `theme_bg`) — TUIs that gate on `4;0;?` (e.g. opentui's `detectOSCSupport`) get their luminance probe honored.
